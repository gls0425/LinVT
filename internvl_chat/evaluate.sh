...
if [ ${DATASET} == "mmbench-dev-en" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/mmbench/evaluate_mmbench.py --checkpoint ${CHECKPOINT} --datasets mmbench_dev_20230712 --out-dir '/mnt/dolphinfs/hdd_pool/docker/user/hadoop-vacv/gaolishuai/results/InternVL2/mmbench-dev-en' "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "mmbench-test-en" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/mmbench/evaluate_mmbench.py --checkpoint ${CHECKPOINT} --datasets mmbench_test_en_20231003 --out-dir '/mnt/dolphinfs/hdd_pool/docker/user/hadoop-vacv/gaolishuai/results/InternVL2/mmbench-dev-en' "${ARGS[@]:2}"
fi
