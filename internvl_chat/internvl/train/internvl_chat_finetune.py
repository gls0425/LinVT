...
class ModelArguments:
    ...
    freeze_ffc: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the FFC layers of the model.'},
    )
    ...
    use_multi_scale: bool = field(
        default=False,
        metadata={'help': 'FFC vit embedding use multi-scale.'}
    )
    use_selector: bool = field(
        default=False,
        metadata={'help': 'Whether use selector for scene_frames sparse.'}
    )
...
class DataTrainingArguments:
    ...
    use_scene_det: Optional[bool] = field(
        default=False,
        metadata={'help': 'Use Scene detect method to split video.'}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        use_scene_det=False, # whether use SceneDet
        use_ffc=False,
        ...
        self.use_scene_det = use_scene_det
    ...
    def video_get_item(self, data_item):
        ...
        image_list, is_feat = self.tcs_loader(
            ...
            use_scene_det=self.use_scene_det)

        # Generate special tokens for each video frame
        if self.use_ffc:
            special_tokens = '\n'.join(['Frame{}: <image>'.format(i + 1) for i in range(self.query_num_token)])
        else:
        ...
        ret = dict(
            input_ids=ret['input_ids'][0],
            value_input_ids=ret['value_input_ids'][0],
            labels=ret['labels'][0],
            attention_mask=ret['attention_mask'][0],
            pixel_values=pixel_values,
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.freeze_ffc = model_args.freeze_ffc
        config.use_multi_scale = model_args.use_multi_scale
        config.use_selector = model_args.use_selector
        config.use_scene_det = data_args.use_scene_det
        ...
        internvl_chat_config = InternVLChatConfig(
            ...
            use_scene_det=data_args.use_scene_det,
    ...
    train_dataset = build_datasets(
        ...
        use_scene_det=data_args.use_scene_det, dynamic_image_size=data_args.dynamic_image_size,
