...
def SceneDetect(video_path, threshold=27.0):
    # 创建一个视频管理器
    video_manager = VideoManager([video_path])
    # 创建一个场景管理器并添加一个内容检测器
    scene_manager = SceneManager()
    scene_manager.add_detector(ContentDetector(threshold=threshold))  # 你可以调整阈值来控制检测敏感度
    # 启动视频管理器并快速获得视频帧实时计数 (frame rate)
    video_manager.set_downscale_factor()  # 下采样因子（如果有性能问题可以设置）
    video_manager.start()
    scene_manager.detect_scenes(frame_source=video_manager)
    # 获取所检测到的场景列表，每个场景是一个 (start, end) 的元组
    scene_list = scene_manager.get_scene_list()
    video_manager.release()
    return scene_list

def read_frames_cv2(video_path, max_num_frames, sample = 'rand', fix_start = None, client = None, clip = None,
                    min_num_frames = 4, use_scene_det=False):
    ...
    if use_scene_det:
        pil_image_scene = []
        # scene detect, larger threshold fewer slices.
        scene_list = SceneDetect(video_path, threshold=30.5)
        scene_num = len(scene_list)
        for i_s, scene in enumerate(scene_list):
            start_frame_scene = scene[0].get_frames()
            end_frame_scene = scene[1].get_frames()
            t_num_frames_scene = (t_num_frames/scene_num) + 1
            vlen_scene = end_frame_scene - start_frame_scene + 1
            frame_indices_scene = get_frame_indices(t_num_frames_scene, vlen_scene, sample=sample,
                                                    fix_start=start_frame_scene, input_fps=fps)
            for frame_idx_scene in frame_indices_scene:
                cv2_vr.set(cv2.CAP_PROP_POS_FRAMES, frame_idx_scene)
                ret_scene, frame_scene = cv2_vr.read()
                if not ret_scene:
                    raise ValueError(f'scene video error at {video_path},frame_idx {frame_idx_scene}')
                frame_scene = cv2.cvtColor(frame_scene, cv2.COLOR_BGR2RGB)
                frame_scene = Image.fromarray(frame_scene)
                pil_image_scene.append(frame_scene)
        cv2_vr.release()
        return pil_image_scene
    ...
    def __call__(self, fn, image_type='image', max_num_frames=-1, min_num_frames=4, sample='rand', clip=None, use_scene_det=False):
        ...
        if image_type == 'image':
            ...
            if fn.endswith('/'):
                ...
                frames = read_frames_cv2(fn, max_num_frames=max_num_frames, min_num_frames=min_num_frames,
...
def preprocess_mpt(
    ...
    prompt_user_value_len, prompt_user_head_len = 0, 0
    # Mask targets. Only compute loss on the assistant outputs.
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, turn in enumerate(re_turns):
            ...
            prompt_user_value = parts[0].split(IMG_END_TOKEN)[-1].split(conv.sep)[0]
            prompt_user_value_len = len(tokenizer(prompt_user_value).input_ids)
            prompt_user_head = parts[0].split(prompt_user_value)[0]
            prompt_user_head_len = len(tokenizer(prompt_user_head).input_ids)
            # print(f'prompt_user_value: ', tokenizer.decode(input_ids[:, prompt_user_head_len: prompt_user_head_len + prompt_user_value_len][0]))
    ...
    return dict(
        input_ids=input_ids,
        value_input_ids=input_ids[:, prompt_user_value_len: prompt_user_value_len+prompt_user_head_len],
        labels=targets,
        attention_mask=input_ids.ne(tokenizer.pad_token_id),
    )
