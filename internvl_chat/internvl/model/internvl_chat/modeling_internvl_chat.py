...
from .linvt import LinearVideoTokenizer
...
class InternVLChatModel(PreTrainedModel):
    ...
    def __init__(self, config: InternVLChatConfig, vision_model=None, language_model=None):
        ...
        self.use_scene_det = config.use_scene_det
        self.frozen_video_FFC = config.freeze_ffc
        # multi-scale on T, in N frames
        self.use_multi_scale = config.use_multi_scale
        self.grid_t_size_list = [3, 4, 5]
        self.grid_scale_list = [0.3, 0.2, 0.17]
        self.use_selector = config.use_selector

        if self.use_video_frames_compress:
            self.query_tokens = nn.Parameter(torch.zeros(1, self.num_video_query_token, config.llm_config.hidden_size))
            self.query_tokens.data.normal_(mean=0.0, std=0.02)
            self.linVT = LinearVideoTokenizer(config.llm_config.hidden_size,4, 8, 1024, use_multi_scale=self.use_multi_scale)
    ...
    def forward(
            ...
            value_input_ids: torch.LongTensor = None,
        ...
        if self.use_video_frames_compress:
            text_input_embeds = self.language_model.get_input_embeddings()(value_input_ids).clone()
            vit_embeds = self.linear_video_tokenizer(vit_embeds, text_input_embeds)
    ...
    def linear_video_tokenizer(self, vit_embeds, text_embeds):
        # add q-former to compress video frames, from N to M.
        time_length = vit_embeds.shape[0]
        # vit_embeds = vit_embeds.mean(dim=1, keepdim=True)
        with self.maybe_autocast():
            ...
            # vit_embeds = einops.rearrange(vit_embeds, 'b t h -> (b t) h')
            vit_embeds = vit_embeds.unsqueeze(0)
            vit_embeds = self.linVT(self.query_tokens, vit_embeds, text_embeds).to(torch.bfloat16).squeeze(0)
            return vit_embeds
